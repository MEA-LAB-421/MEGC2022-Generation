...
class GeneratorFullModel(torch.nn.Module):
    ...
    def forward(self, x, epoch):
        ...
        kp_source = self.kp_extractor(x['keypoint'])
        kp_driving = self.kp_extractor(x['keypoint'])
        bg_param = None
        ...
        if isinstance(self.inpainting_network, InpaintingNetwork_AU):
            generated = self.inpainting_network(x['source'], dense_motion, x['AU_diff'])
        else:
        ...
        if self.loss_weights['equivariance_value'] != 0:
            ...
            transformed_kp = self.kp_extractor(x['keypoint'])
