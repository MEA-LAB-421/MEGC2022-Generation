...
torch.set_num_threads(20) # 核心数限制为20以内

def train(config, inpainting_network, kp_detector, bg_predictor, dense_motion_network, checkpoint, log_dir, dataset):
    train_params = config['train_params']
    optimizer = torch.optim.Adam(
        [{'params': list(inpainting_network.parameters()) + list(dense_motion_network.parameters()) + list(kp_detector.parameters()),
          'initial_lr': train_params['lr_generator']}], lr=train_params['lr_generator'], betas=(0.5, 0.999), weight_decay=1e-4)
    ...
    with Logger(log_dir=log_dir, visualizer_params=config['visualizer_params'],
        ...
        for epoch in trange(start_epoch, train_params['num_epochs']+start_epoch):
            for x in tqdm(dataloader):
                if(torch.cuda.is_available()):
                    x['driving'] = x['driving'].cuda()
                    x['source'] = x['source'].cuda()
                    if config['dataset_params']['keypoint_dir']:
                        x['keypoint'] = x['keypoint'].cuda()
                    if config['dataset_params']['au_dir']:
                        x['AU_diff'] = x['AU_diff'].cuda()
